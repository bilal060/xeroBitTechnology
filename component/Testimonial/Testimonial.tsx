...
const Testimonial = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await CallAPI("contactus");
      const jsonResponse = await response.data;
      if (jsonResponse) {
        setArrayData(jsonResponse?.contact);
      ...
      <section className="testimonials">
        ...
        <div className="container pt-5">
          <div className="tes-conent">
            <h6 className="section-first-heading text-center font-weight-500 pt-2 pb-1">
              testimonials
            </h6>
            <h2 className="mb-4 mt-3 section-sub-heading text-capital text-center text-black font-weight-700">
              what our clients say about us!
            </h2>
            <p className="pt-4 font-16 text-dark-gray text-center mx-auto ">
              Lorem ipsum dolor sit amet, consectetur adipiscing elit.
              Vestibulum nec vulputate lectus, in accumsan <br /> leo. Aliquam
              tempor orci sed ex facilisis vestibulum.
            </p>
          </div>
        </div>
            ...
            {arrayData.map((val: any, index) => {
              if (val._id) {
                return (
                  <div key={index}>
                    <TestimonialCard>
                      <p className="mb-4">{val.massage}</p>
                      <div className="d-flex justify-content-between align-items-end gap-16px pt-2">
                        <div className="d-flex justify-content-start align-items-center gap-16px">
                          <Image src={comment2} alt="" className="gap-16px" />
                          <div>
                            <p className="m-0 font-16 font-weight-700">
                              {val.firstName}
                            </p>
                            <p className="m-0 font-16 font-weight-400">
                              Google
                            </p>   
                          </div>      
                        </div>
